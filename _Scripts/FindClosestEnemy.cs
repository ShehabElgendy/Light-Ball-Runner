...
public class FindClosestEnemy : MonoBehaviour
{
    ...
    private void Awake()
    {
        garbageSpawner = FindObjectOfType<GarbageSpawner>();
    }
    private void Start()
    {
    ...
    public float FirePowerUp(float increaseAmount)
    {
        if (fireWaitTime1 > 0.5f)
        {
            ...
            fireWaitTime1 = 0.5f;
        return fireWaitTime1;
    }



    public float DecreaseFireAndSpeed(float decreaseFireRateAmount)
    {
        if (fireWaitTime1 < 1f)
